          ...
          <div className="p-4 h-rate hovered border border-border bg-dry rounded-lg overflow-hidden">
            ...
            <div className="px-4 hoveres gap-6 text-center absolute bg-black bg-opacity-70 top-0 left-0 right-0 bottom-0">
              ...
              >
                {movie?.name} 
                <br />
                <span classNames="text-ml">{movie?.platform }</span>
              </Link>
...
function Netflix({ movies, isLoading }) {
  ...
  return (
    <div className="my-16">
      <Titles title="Recomendaciones Netflix" Icon={BsBookmarkStarFill} />
      <div className="mt-10">
        {isLoading ? (
          <Loader />
        ) : movies?.length > 0  ? (
          ...
          <Empty message="Este elemento no contiene nada" />
        )}
