...
//lista de objetosd de pelicula
const ReviewItem = ({ review, onRemoved }) => {
  const [onRequest, setOnRequest] = useState(false);
//primero sincronizamos los datos para ver las reviews de las peliculas disponibles
  const onRemove = async () => {
    //se verifica si en estas si tiene datos
    if (onRequest) return;
    setOnRequest(true);
    const { response, err } = await reviewApi.remove({ reviewId: review.id });
    setOnRequest(false);
//de no ser asi mandamos un mensaje de error
    if (err) toast.error(err.message);
    if (response) {
      toast.success("Review retirada correctamente");
      onRemoved(review.id);
    }
  };
...
//para ver tus reviews desde tu perfil
const ReviewList = () => {
